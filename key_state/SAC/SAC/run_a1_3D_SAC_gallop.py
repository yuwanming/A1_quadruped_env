...
from SAC.SAC.configuration_gallop import *
from SAC.agent.agent_SAC import *
from SAC.common.logger import logger

from a1_gallop_phase_env_key_state import A1
...
class Run():
    def __init__(self, config, dir_path, args):
        ...
        self.clip_bounds = np.array([
            [-0.2617, -2.7923, 0.9162, -0.2617, -2.7923, 0.9162, -0.2617, -2.7923, 0.9162, -0.2617, -2.7923, 0.9162],
            [0.2617, 1.0471, 2.6963, 0.2617, 1.0471, 2.6963, 0.2617, 1.0471, 2.6963, 0.2617, 1.0471, 2.6963]
        ])
    ...
    def test(self):
        ...
        for i in range(1):#self.config.conf['test-num']):
            ...
            for step in range(self.max_step_per_test_episode):
                ...
                rgb=self.env._render(distance=2, pitch=-10, yaw=45)
...
def main():
    ...
    dir_path = parentdir + '/SAC/SAC/SAC/record/a1/gallop/' + args.folder
