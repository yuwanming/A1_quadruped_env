...
class A1(gym.Env):
    ...
    def __init__(self,
        ...
        self.stateNumber = 25+3+len(self.controlled_joints)*3
    ...
    def _reset(self, base_pos_nom = None, base_orn_nom = None, fixed_base = False, q_nom = None, g = 9.81):
    ...
    def task_reward(self):
        ...
        base_x_vel_err = np.maximum(base_x_vel_tar - base_pos_vel_yaw[0][0] ,0.0)
        ...
        # base_z_pos_reward = math.exp(math.log(alpha)*(base_z_pos_err/1.0)**2)
        ...
        foot_in_base_tar = np.array([0.18, 0.13, -0.18, -0.13, -0.18, 0.13, 0.18, -0.13])
        foot_in_base = np.array([fl_base[0][0], fl_base[0][1], rr_base[0][0], rr_base[0][1], rl_base[0][0], rl_base[0][1], fr_base[0][0], fr_base[0][1]])
        foot_in_base_err = np.linalg.norm(foot_in_base - foot_in_base_tar)


        foot_in_base_reward = math.exp(math.log(alpha) * (foot_in_base_err / 0.3) ** 2)
        foot_in_base_reward = foot_in_base_reward * base_gravity_vector_reward * base_z_pos_reward
        ...
        base_xy_pos_reward = base_xy_pos_reward * base_gravity_vector_reward
        ...
        reward = (
                    ...
                    + 1.*base_z_vel_reward \
                    + 5.*base_z_pos_reward \
                    + 5.*foot_in_base_reward \
                    + 5.*base_gravity_vector_reward \
                    + 2.*joint_vel_reward \
                    + 2.*joint_torque_reward \
                     ) \
                * 10. / (1.0+1.0+1.0+5.0+5.0+5.0+2.0+2.0)
        ...
        reward_term = dict([
            ...
            ("foot_in_base_reward", foot_in_base_reward),
    ...
    def checkFall(self):
        ...
        #remove foot from checklist
        link.remove(self.robotConfig.ground_contact_link[0])
        link.remove(self.robotConfig.ground_contact_link[1])
        link.remove(self.robotConfig.ground_contact_link[2])
        link.remove(self.robotConfig.ground_contact_link[3])
    ...
    def _setupCamera(self, cameraDistance=1.5, cameraYaw=0, cameraPitch=0, cameraTargetPosition=np.array([0, 0, 0.3])):
        p.configureDebugVisualizer(p.COV_ENABLE_RENDERING, 1)
        p.configureDebugVisualizer(p.COV_ENABLE_RGB_BUFFER_PREVIEW,0)
    ...
    def getObservation(self):
        ...
        self.getEEF() # EEF related stuff is set here
        #
        x_observation[9]  = self.fl_lower[0][0]
        x_observation[10] = self.fl_lower[0][1]
        x_observation[11] = self.fl_lower[0][2]
        x_observation[12] = self.fr_lower[0][0]
        x_observation[13] = self.fr_lower[0][1]
        x_observation[14] = self.fr_lower[0][2]
        x_observation[15] = self.rl_lower[0][0]
        x_observation[16] = self.rl_lower[0][1]
        x_observation[17] = self.rl_lower[0][2]
        x_observation[18] = self.rr_lower[0][0]
        x_observation[19] = self.rr_lower[0][1]
        x_observation[20] = self.rr_lower[0][2]
        x_observation[21] = self.fl_force_smooth
        x_observation[22] = self.fr_force_smooth
        x_observation[23] = self.rl_force_smooth
        x_observation[24] = self.rr_force_smooth

        x_observation[25] = base_pos[0]
        x_observation[26] = base_pos[1]
        x_observation[27] = base_pos[2]
        ...
        for i in range(self._actionDim):
            ...
            joint_state = p.getJointState(self.r, self.jointIdx[name])
            x_observation[index] = joint_state[0] # position
            x_observation[index+self._actionDim] = joint_state[1] / self.v_max[name]  # velocity normalized by max vel
            x_observation[index+self._actionDim*2] = joint_state[3] / self.u_max[name] # torque normalized by max torque
    ...
    def getEEF(self):
        ...
        self.fl_force, self.fr_force, self.rl_force, self.rr_force = self.calTerrainContactForce()
    ...
    def calTerrainContactForce(self):
        footGroundContact = []
        ankleRollContact = self._p.getContactPoints(self.r, self.plane, self.jointIdx[self.robotConfig.ground_contact_link[0]], -1)
        footGroundContact.extend(ankleRollContact)
        for i in range(len(self.obstacleID)):
            ankleRollTerrainContact = p.getContactPoints(self.r, self.obstacleID[i], self.jointIdx[self.robotConfig.ground_contact_link[0]], -1)
            footGroundContact.extend(ankleRollTerrainContact)
        fl_contact_info = footGroundContact
        ...
        footGroundContact.extend(ankleRollContact)
        for i in range(len(self.obstacleID)):
            ankleRollTerrainContact = p.getContactPoints(self.r, self.obstacleID[i], self.jointIdx[self.robotConfig.ground_contact_link[1]], -1)
            footGroundContact.extend(ankleRollTerrainContact)
        fr_contact_info = footGroundContact
        ...
        footGroundContact.extend(ankleRollContact)
        for i in range(len(self.obstacleID)):
            ankleRollTerrainContact = p.getContactPoints(self.r, self.obstacleID[i], self.jointIdx[self.robotConfig.ground_contact_link[2]], -1)
            footGroundContact.extend(ankleRollTerrainContact)
        rl_contact_info = footGroundContact
        ...
        footGroundContact.extend(ankleRollContact)
        for i in range(len(self.obstacleID)):
            ankleRollTerrainContact = p.getContactPoints(self.r, self.obstacleID[i], self.jointIdx[self.robotConfig.ground_contact_link[3]], -1)
            footGroundContact.extend(ankleRollTerrainContact)
        rr_contact_info = footGroundContact
