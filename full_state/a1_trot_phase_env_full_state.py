...
from a1_description.a1_trot_configuration import *
...
class A1(gym.Env):
    ...
    def __init__(self,
        ...
        self.stateNumber = 25+2+3+len(self.controlled_joints)*3
    ...
    def task_reward(self):
        ...
        base_x_vel_tar = 0.5
        base_y_vel_tar = 0.0
        base_xy_vel_tar = np.array([0.5, 0.0])
        base_z_vel_tar = 0.0
        ...
        base_xy_vel_reward = math.exp(math.log(alpha)*(base_xy_vel_err/0.25)**2)
        base_x_vel_reward = math.exp(math.log(alpha)*(base_x_vel_err/0.25)**2)
        base_y_vel_reward = math.exp(math.log(alpha)*(base_y_vel_err/0.25)**2)
        base_z_vel_reward = math.exp(math.log(alpha)*(base_z_vel_err/0.25)**2)
        ...
        fl_pos_z = fl[0][2]
        self.fl_z = fl_pos_z
        fl_err = (fl_pos_z - foot_z)/0.1*fl_vel_xy
    ...
    def imitation_reward(self):
        ...
        if gait_phase >= 0 and gait_phase < gap:
            if (fl_ground_contact == True and fr_ground_contact == True and rl_ground_contact == True and rr_ground_contact == True): #allow double contact
                contact_term = 2
            else:
                contact_term = 0
        elif gait_phase >= gap and gait_phase < 0.5:
            if (fl_ground_contact == True and fr_ground_contact == False and rl_ground_contact == False and rr_ground_contact == True):
                contact_term = 2
            else:
                contact_term = 0
            ...
            if (fl_ground_contact == False and fr_ground_contact == True and rl_ground_contact == True and rr_ground_contact == False):
                contact_term = 2
            else:
                contact_term = 0
        else:
    ...
    def getObservation(self):
        ...
        x_observation[25] = phase_sin
        x_observation[26] = phase_cos

        x_observation[27] = base_pos[0]
        x_observation[28] = base_pos[1]
        x_observation[29] = base_pos[2]
    ...
    def getFilteredObservation(self):
        ...
        observation_filtered[25:27] = self.unfiltered_phase
    ...
    def performFiltering(self):
        ...
        self.unfiltered_phase = np.array([observation[25], observation[26]])
