...
from SAC.SAC.configuration_standup import *
from SAC.agent.agent_SAC import *
from SAC.common.logger import logger

from a1_standup_env_full_state import A1
...
class Train():
    def __init__(self, config):
        ...
        self.best_front_reward = 0
        self.best_back_reward = 0
        self.best_left_reward = 0
        self.best_right_reward = 0

        # create new network
        dir_path = 'SAC/record/a1/standup/' + datetime.now().strftime('%Y_%m_%d_%H.%M.%S')
    ...
    def test(self):
        ...
        individual_reward = np.zeros(self.config.conf['test-num'])
        for i in range(4):#
            if i == 0:
                # back
                ...
                q_nom = dict([
                    ...
                    ("FR_lower_leg_2_upper_leg_joint", 2.69),  # [-0.23, 0.255]
                    ("FL_hip_motor_2_chassis_joint", 0),
                    ("FL_upper_leg_2_hip_motor_joint", -1.66),
                    ("FL_lower_leg_2_upper_leg_joint", 2.69),
                    ("RR_hip_motor_2_chassis_joint", 0),
                    ("RR_upper_leg_2_hip_motor_joint", -1.66),
                    ("RR_lower_leg_2_upper_leg_joint", 2.69),
                    ("RL_hip_motor_2_chassis_joint", 0),
                    ("RL_upper_leg_2_hip_motor_joint", -1.66),
                    ("RL_lower_leg_2_upper_leg_joint", 2.69),
                ])
            elif i==2:
                # left
                base_pos_nom = [0, 0, 0.17]
                ...
                #back
                base_pos_nom = [0, 0, 0.08]
                base_orn_nom = euler_to_quat(0, 3.14, 0)  # euler_to_quat(-1.57,0,0)#euler_to_quat(0,1.57,0)
                q_nom = dict([
                    ("FR_hip_motor_2_chassis_joint", 0),  # [-1.329, 1.181]
                    ("FR_upper_leg_2_hip_motor_joint", -0.74),
                    ("FR_lower_leg_2_upper_leg_joint", 1.69),  # [-0.23, 0.255]
            ...
            for step in range(self.max_step_per_test_episode):
                ...
                if terminal:
                    break
            individual_reward[i] = total_reward
        ave_reward = sum(individual_reward)/self.config.conf['test-num']
        ...
        self.logging.add_train('back_reward', individual_reward[0])
        self.logging.add_train('front_reward', individual_reward[1])
        self.logging.add_train('left_reward', individual_reward[2])
        self.logging.add_train('right_reward', individual_reward[3])
